...
```bash
# ZIP 추출 후 각 CSV 업로드
curl -s -u tobe:tobe1245 -F "file=@result.csv;filename=result.csv" -F "account=keychron" "http://13.125.219.231:8087/api/gfa/upload"
```

### 검증
```bash
curl -s -u tobe:tobe1245 "http://13.125.219.231:8087/api/gfa/data-status?account=keychron" | python3 -m json.tool
...
6. 업로드: `curl -s -u tobe:tobe1245 -F "file=@result.csv" -F "account=keychron" http://13.125.219.231:8087/api/gfa/upload`
