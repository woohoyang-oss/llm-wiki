...
### 스위치 선호도 분석 (바나나축 발주 비율)

키크론 제품군의 바나나축 옵션(저소음/라이트 저소음/일반)별 판매 비율을 분석하여 발주 비율 인사이트를 제공한다.

**API 엔드포인트**: `GET /api/v1/sales/switch-preference`

| 파라미터 | 설명 | 기본값 |
|----------|------|--------|
| `account` | 스토어 구분 | keychron |
| `start_date` | 시작일 | 3개월 전 |
| `end_date` | 종료일 | 오늘 |
| `model_keyword` | 모델 키워드 (예: K10 Pro SE) | 빈값=전체 |

#### 스위치 타입 분류 CASE

```sql
-- 바나나축 3종 분류 (product_name 기반)
-- ★ 순서 중요: '라이트 저소음'이 '저소음'보다 먼저 매칭되어야 함
CASE
    WHEN product_name LIKE '%라이트 저소음%바나나%' THEN '라이트 저소음 바나나'
    WHEN product_name LIKE '%저소음%바나나%'       THEN '저소음 바나나'
    WHEN product_name LIKE '%바나나%'
      OR product_name LIKE '%Banana%'               THEN '일반 바나나'
    ELSE '기타'
END
```
...
WHERE (product_name LIKE '%바나나%' OR product_name LIKE '%Banana%')
  ...
  AND account = %s
  AND order_date >= %s AND order_date < %s
GROUP BY month, switch_type
ORDER BY month, switch_type
```

#### 스위치별 재고 + 판매속도 SQL

```sql
-- ERP 재고 기반 바나나축 옵션별 재고 소진 속도
-- GT창고(G-code) + CJ창고(F-code) 합산
SELECT
    p.goods_name,
    p.option_name,
    g.productCode AS product_code,
    g.ea + IFNULL(f.ea, 0) AS total_stock,
    g.beDueEa + IFNULL(f.beDueEa, 0) AS incoming,
    g.oneMonthSalesEa + IFNULL(f.oneMonthSalesEa, 0) AS monthly_sales,
    g.threeMonthSalesEa + IFNULL(f.threeMonthSalesEa, 0) AS quarterly_sales,
    ROUND((g.oneMonthSalesEa + IFNULL(f.oneMonthSalesEa, 0)) / 30.0, 1) AS daily_avg,
    CASE
        WHEN (g.oneMonthSalesEa + IFNULL(f.oneMonthSalesEa, 0)) > 0
        THEN ROUND((g.ea + IFNULL(f.ea,0) + g.beDueEa + IFNULL(f.beDueEa,0))
...
WHERE g.productCode LIKE 'G-%'
  AND p.sell_status != 'N'
  AND (p.option_name LIKE '%바나나%' OR p.option_name LIKE '%Banana%')
  AND (p.goods_name LIKE %s OR p.option_name LIKE %s)
...
```
Q: "K10 Pro SE 레트로파스텔 발주할 때 바나나축 비율은?"

응답 예시:
[스위치별 판매 비율]
  저소음 바나나: 77% (최근 3개월 기준)
  일반 바나나: 14%
  라이트 저소음 바나나: 9% (신규, 상승 추세)
...
[긴급 발주 필요 (2주 미만)]
  저소음바나나: 재고 46개 / 월판매 120개 / 12일분
[주의 (1개월 미만)]
  라이트저소음바나나: 재고 111개 / 월판매 123개 / 27일분
```

**관련 테이블**: `naver_smartstore_orders` (판매 비율), `erp_stock_realtime` + `erp_product_info` (재고/속도)
**핵심 컬럼**: `product_name` (스위치 분류), `option_name` (재고 검색)
...
#### Q: "XX 추가 발주 해야해?" / "XX 발주 얼마나 해야해?" — ★★★ 발주 분석 종합 프롬프트
...
"XX 추가발주 해야해?", "XX 발주 필요해?", "XX 발주 얼마나?"
같은 질문을 받으면 아래 순서대로 API를 호출하고 결과를 조합한다.

────────────────────────────────────────────────────────────────
[Step 1] API 호출 3건 (병렬 가능)
────────────────────────────────────────────────────────────────

  ① 발주 추천 분석
     GET /api/v1/inventory/order-recommendation?keyword={XX}
     → 옵션별: 가중일판매, 5-tier 상태, 추천수량, 소진일, 매출손실

  ② 출하촉진 확인
     GET /api/v1/inventory/shipment-expedition?keyword={XX}
     → 기존 PO: 경과일, beDueEa, expedition_status (URGENT/CHECK/OK)

  ③ 옵션 불균형 확인
     GET /api/v1/inventory/sibling-analysis?keyword={XX}
     → 형제 옵션: imbalanced 여부, 과잉 옵션 프로모션 필요성

────────────────────────────────────────────────────────────────
[Step 2] 응답 포맷 — 반드시 아래 구조로 출력
────────────────────────────────────────────────────────────────
...
| 옵션 | 재고 | 가중일판매 | 소진일 | 상태 | 기존PO | 추천발주 |
|------|------|-----------|--------|------|--------|---------|
(① 응답의 items 배열을 테이블로 출력)

  컬럼 매핑:
  - 옵션: option_name에서 "(코드)" 추출 (예: "EX75-A6")
  - 재고: total_stock + "ea"
  - 가중일판매: weighted_daily_sales + "ea/일"
  - 소진일: days_of_supply + "일"
  - 상태: status (긴급/주의/안전/과잉/발주중) + 이모지
    긴급=🔴, 주의=🟡, 안전=✅, 과잉=⚪, 발주중=🔵
  - 기존PO: pending_order_qty + "ea"
  - 추천발주: recommended_qty + "ea" (0이면 "0ea")
...
### 핵심 판단
  ...
  **2️⃣ 출하촉진: {결론}**
  → ② 응답 기반, 경과일 + beDueEa + status

  **3️⃣ 신규/미판매 옵션: {있으면 언급}**
  → 판매 데이터 없는 옵션은 "수요 미확인" 명시
...
────────────────────────────────────────────────────────────────
  ...
  IF 전 옵션 상태 == "안전" 또는 "과잉":
    → "❌ 추가 발주 불필요"
    → 근거: 가용재고 N일분, 기존 PO Nea 입고 시 N일분

  ELIF 긴급/주의 옵션 존재 AND 기존 PO 없음:
    → "🔴 즉시 발주 필요"
    → 추천 수량: recommended_qty 합계
    → 근거: 가중일판매 Nea/일, 소진일 N일, 리드타임 N일

  ELIF 긴급/주의 옵션 존재 AND 기존 PO 있음 (상태="발주중"):
    → "🟡 기존 PO 입고 확인 후 재판단"
    → 근거: PO Nea 입고 시 N일분 추가 확보

  ELIF 일부 옵션만 긴급 (③에서 imbalanced=true):
    → "⚠️ 옵션별 차등 발주 검토"
    → 과잉 옵션은 프로모션, 긴급 옵션만 발주

  ★ 출하촉진 판단:

  IF ②에서 expedition_status == "URGENT" 존재:
    → "🔴 즉시 출하촉진 필요"
    → 근거: N일 경과 + beDueEa=0 + due_date={값}

  ELIF ②에서 expedition_status == "CHECK" 존재:
    → "🟡 출하 일정 확인 필요"

  ELSE:
    → 해당 없음 (출하촉진 불필요)

  ★ PO 미입고 시 시뮬레이션 (필수 포함):
    시나리오A: 현재고만 / 가중일판매 = N일분
    시나리오B: (현재고+PO수량) / 가중일판매 = N일분
    → "PO N건 입고 시 N일분 확보" 형태로 표시

────────────────────────────────────────────────────────────────
[Step 4] 가중 일판매량 계산 공식 (v3 DevService)
────────────────────────────────────────────────────────────────

  weighted_daily_sales = (주간판매/7 × 0.5)
                       + (월간판매/30 × 0.3)
                       + (분기판매/90 × 0.2)

  ⚠️ 왜 가중치를 쓰나?
  - 단순 월/30은 성장기 상품의 현재 속도를 과소평가
  - 주간(최근 7일)에 50% 가중 → 급증/급감 즉시 반영
  - 예: 주130, 월334, 분기408
    → 가중: 13.54ea/일 vs 단순: 11.13ea/일 (21% 차이)

────────────────────────────────────────────────────────────────
[Step 5] 5-tier 상태 판별 기준
────────────────────────────────────────────────────────────────

  ┌──────────┬────────────────────────────────────┬────────────┐
  │ 상태     │ 조건                                │ 긴급도     │
  ├──────────┼────────────────────────────────────┼────────────┤
  │ 긴급     │ DOS ≤ 유효리드타임                  │ 50~100     │
  │ 주의     │ 리드타임 < DOS ≤ 1.5×리드타임       │ 0~50       │
  │ 안전     │ 1.5×리드타임 < DOS < 과잉기준       │ 0          │
  │ 과잉     │ DOS ≥ 과잉기준 OR 일판매 ≤ 0.01    │ 0          │
  │ 발주중   │ 긴급/주의이지만 PO 존재             │ 50% 감소   │
  └──────────┴────────────────────────────────────┴────────────┘

  동적 안전재고  = MAX(14, 리드타임×0.5) × (급증 시 1.3)
  과잉기준      = MAX(90, (리드타임+안전일수)×3)
  추천발주수량  = CEIL((일판매×(리드타임+안전일수)) - 가용재고, 10단위)
  매출손실추정  = 일판매 × 매입가 × 1.5 × 품절예상일수 (긴급 시만)

────────────────────────────────────────────────────────────────
[Step 6] IQR 이상치 캡핑 (선택, apply_iqr=true 시)
────────────────────────────────────────────────────────────────

  MONTHLY_SALES_HISTORY (24개월 월별 출고) →
  Q1/Q3/IQR → upper_bound = Q3 + 1.5×IQR →
  이상치 캡핑 → monthly 재계산 → 가중일판매 재계산

  ⚠️ 프로모션/이벤트로 특정 월 판매 급등 시 이상치 제거 효과

────────────────────────────────────────────────────────────────
[금지 사항]
────────────────────────────────────────────────────────────────

  ❌ PO 미확인 상태에서 "즉시 발주 권장" 금지
  ❌ 단순 월판매/30으로 소진일 계산 금지 → 반드시 가중일판매 사용
  ❌ GT 창고만 보고 "품절" 판단 금지 → GT+CJ 합산 필수
  ❌ 옵션 전체 합산만으로 판단 금지 → 옵션별 개별 확인 필수
  ❌ beDueEa=0인 PO를 "입고 예정"으로 착각 금지

════════════════════════════════════════════════════════════════
★ 실제 사례: EX75 HE 분석 (2026-03-02)
════════════════════════════════════════════════════════════════

[API ① 결과] order-recommendation?keyword=EX75

  | 옵션 | 재고 | 가중일판매 | 소진일 | 상태 | 기존PO | 추천발주 |
  |------|------|-----------|--------|------|--------|---------|
  | EX75-A6 (블랙 저소음) | 542ea | 13.53ea/일 | 40일 | ✅ 안전 | 500ea | 0ea |
  | EX75-A1 (블랙 일반) | 0ea | 0 | — | ⚪ 신규 | 500ea | 0ea |
  | EX75-J6 (샤인스루 저소음) | 0ea | 0 | — | ⚪ 신규 | 500ea | 0ea |
  | EX75-J1 (샤인스루 일반) | 0ea | 0 | — | ⚪ 신규 | 500ea | 0ea |

[API ② 결과] shipment-expedition?keyword=EX75

  K260123 | 4건 × 500ea | 38일 경과 | beDueEa=0 | 전건 🔴 URGENT

[판단 적용]

  1️⃣ 추가 발주: ❌ 불필요
     → A6: 가용 542ea ÷ 13.53 = 40일분 (안전)
     → PO 입고 시: (542+500) ÷ 13.53 = 77일분
     → 급증 중(trend 4.1×)이지만 PO 500ea로 커버

  2️⃣ 출하촉진: 🔴 즉시 필요!
     → 38일 경과 + beDueEa=0 + due_date=NULL
     → 4건 전부 URGENT

  3️⃣ 신규 3종(A1, J6, J1): 수요 미확인
     → 입고 후 2~3주 초기 반응 모니터링 필요

  액션 아이템:
  1. 키크론 본사에 PO K260123 출하 일정 즉시 확인
  2. 입고 후 A6 판매 가속도 재점검 → 3월 중순 재판단
  3. 신규 3종 입고 후 초기 판매 모니터링 → 추가 발주 여부 결정
```

**관련 API**: `/order-recommendation` + `/shipment-expedition` + `/sibling-analysis`
**핵심 원칙**: ① PO 없이 발주 권장 금지 ② 가중일판매 사용 필수 ③ 옵션별 개별 확인 ④ beDueEa=0이면 출하촉진 제안

#### Q: "이 상품 B2C vs B2B 비율이 어떻게 돼?" — ★ 채널별 판매량 분석
...
API: GET /api/v1/inventory/channel-sales?keyword={XX}

응답 포맷:
  | 옵션 | 전체(1m) | B2C | B2B | 로켓 | B2C% | B2B% | 로켓% |
  (items 배열 → 테이블)

해석 가이드:
  - B2C 80%+ → 스마트스토어 추세 분석(trend-analysis) 신뢰도 높음
  - B2B 50%+ → 스마트스토어 추세만으로 판단 위험, ERP 출고 기준 병행
  - 로켓 30%+ → 쿠팡 로켓 재고 별도 관리 필요성 검토

데이터 소스: erp_stock (export_date 기반 출고 COUNT)
├── B2C: erp_partner.partner_target = 'B2C'
├── B2B: partner_target = 'B2B' AND code ≠ ptn0091
├── Rocket: ptn0091 (쿠팡로켓)
└── 제외: ptn0000/0001/0100/0103/0124 (자가이동/반품)
```
**API**: `/api/v1/inventory/channel-sales`
**파라미터**: `keyword` (필수)

#### Q: "발주 했는데 언제 들어오지?" — ★ 출하촉진 필요 발주 확인
...
API: GET /api/v1/inventory/shipment-expedition?keyword={XX}

응답 포맷:
  각 PO별:
  {po_number} | {옵션} | {수량}ea | {발주일} | {경과일}일 | beDue:{N} | {상태}

상태 판별:
  🔴 URGENT: 30일+ 경과 AND beDueEa = 0
  🟡 CHECK:  14일+ 경과 AND beDueEa = 0
  ✅ OK:     beDueEa > 0 (출하 진행 중)

해석 가이드:
  URGENT → "키크론 본사에 즉시 출하 일정 확인"
  CHECK  → "출하 진행 여부 확인 필요"
  OK     → "입고 대기 중 (정상)"
```
**API**: `/api/v1/inventory/shipment-expedition`
**파라미터**: `keyword` (필수)

#### Q: "XX 옵션별로 어떤 상태야?" — ★ 옵션 그룹 불균형 분석
...
API: GET /api/v1/inventory/sibling-analysis?keyword={XX}
...
응답 포맷:
  ...
  recommendation = "{API 응답 그대로}"

해석 가이드:
  imbalanced=true → "⚠️ 불균형: 긴급 옵션과 과잉 옵션 공존"
    → 과잉 옵션 프로모션 소진 후 발주 권장
    → 전체 합산 발주하면 과잉 옵션이 더 쌓임

  imbalanced=false AND urgent_count > 0 → "발주 필요"
  imbalanced=false AND urgent_count = 0 → "안전 상태"
```
**API**: `/api/v1/inventory/sibling-analysis`
**파라미터**: `keyword` (필수)
**API**: `/api/v1/inventory/sibling-analysis`
**파라미터**: `keyword` (필수)
**관련 테이블**: `erp_stock_realtime` + `erp_product_info`
...
-- ★ 바나나축 모델 발주 시 옵션별 비율은
-- API: GET /api/v1/sales/switch-preference?model_keyword=K10+Pro+SE
-- 참조: "스위치 선호도 분석" 섹션
```
