...
- **Claude Team 세션 이관**: [[auto-email]] 등 프로젝트에서 에이전트 간 세션 이관 패키지를 vault에 저장/로드하여 컨텍스트를 유지한다.
- **크로스 세션 컨텍스트**: 대화 세션이 종료되어도 vault에 저장한 데이터를 다음 세션에서 불러와 연속 작업이 가능하다.
- **보안**: 모든 vault 데이터는 AES-256-GCM으로 암호화되며, 사용자별 파생 키로 격리된다.
- **다중 인터페이스**: REST API, MCP 서버, WebSocket을 모두 지원하여 Claude, ChatGPT 등 다양한 AI에서 접근 가능하다.
